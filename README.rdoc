...
implementations (modules). Please, consider to take a look on the realize
...
* activesupport (version 1.0.0 or later)
* require_all (version 1.0.0 or later)

=== Extra Development Dependencies

* newgem (version 1.4.1 or later)
* cucumber (version 0.3.11 or later)

== Installation

=== Gem Installation

The preferred method of the Most [Core] installation is through
...
* gem install most
* gem install most --include-dependencies

Not that you must have correspondent account privileges
...
Most [Core] can be used as a standalone command line application
or as a library. Due to the modular nature of the system please
consider to read documentation provided with each interface implementation.

=== Library Usage

To get access to the core interface of the system you need to call
the +init+ method first:

	require ‘most’

    most_controller = Most.init

If the initialization went well, the +init+ method will return a reference to
the new +MostController+ instance through which you can control the entire
testing system.

=== Configuration

The default path for the configuration files is _.most_ directory under
the system user’s home folder.

=== Command Line Usage

The list of command line options can be found in the system documentation
or it can be obtained from the Most executable by one of the following calls:
...
Most [Core] is currently hosted at RubyForge and GitHub.
...
* http://github.com/#{github_username}/#{project_name}.git

http://rubyforge.org/projects/most/

== Additional Information

Author:: Dmitriy Toksaitov <toksaitov.d@gmail.com>
Forum:: 
